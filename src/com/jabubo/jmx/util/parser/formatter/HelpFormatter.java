package com.jabubo.jmx.util.parser.formatter;
...
// TODO it would be nice to group the parameters into categories so that we can display "universal attributes" and command-specific attributes separately

public class HelpFormatter {

	private static final Logger LOGGER = Logger.getLogger(HelpFormatter.class);
	
	// TODO redesign this - strategy pattern or something like this?
	...
	public String getSyntaxLine(ParameterParser parser) {
		...
		for (Param param : params) {
			...
			if (formatForWeb)
				exampleValueFormatter = param.new WebExampleValueFormatter();
	...
	public String getParameterDetails(ParameterParser parser) {
		...
		for (Param param : params) {
			...
			if (formatForWeb) {
				sb.append("<li>");
				sb.append(
						formatter.format("<tt>%s</tt>&nbsp;%s", 
								exampleValueFormatter.getExampleValue(null), 
